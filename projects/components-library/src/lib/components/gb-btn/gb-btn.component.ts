...
@Component({
  ...
  imports: [IonIcon, IonSpinner, GbIconComponent],
})
export class GbBtnComponent {
  ...
  label = input("");
  color = input<
    "blue" | "pink" | "error" | "warning" | "success" | "cyan" | "yellow" | "lime" | "gray-dark" | "stroke" | "white"
  >("blue");
  ...
  classes = computed(() => {
    ...
    if (this.fill() == "solid")
      classes += ` disabled:bg-gb-gray-light-500 disabled:text-gb-dark-600 bg-gb-${color}-500 text-white active:bg-gb-${color}-600 border-gb-${color}-500 active:border-gb-${color}-600 hover:border-gb-${color}-400`;
